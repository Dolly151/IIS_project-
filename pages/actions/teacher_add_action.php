...
if ($courseId > 0 && $userId > 0) {
    ...
    if ($svc->addTeacher($courseId, $userId)) {
        redirect('/pages/course_teachers.php?id='.$courseId.'&success='.urlencode('Lektor přidán.'));
        exit;
    }
    // buď není lektor, nebo už existuje vazba (unikátní klíč)
    redirect('/pages/course_teachers.php?id='.$courseId.'&error='.urlencode('Uživatel není lektor nebo už je v kurzu.'));
    exit;
}
