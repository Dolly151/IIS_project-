...
foreach ($items as $it) {
    ...
    if ($dayIdx < 1 || $dayIdx > 5)
        continue; // zobrazujeme Po–Pá

    // ořízni mimo okno (8–18)
    $startClamp = clone $start;
    $endClamp = clone $end;
    if ((int) $startClamp->format('H') < $startHour)
        $startClamp = new DateTime($start->format('Y-m-d') . sprintf(' %02d:00:00', $startHour));
    if ((int) $endClamp->format('H') >= $endHour)
        $endClamp = new DateTime($start->format('Y-m-d') . sprintf(' %02d:00:00', $endHour));
    ...
    $byDay[$dayIdx][] = [
        ...
        'term_id' => $it['term_id'] ?? null,   // ← DŮLEŽITÉ
    ];
        ...
        <main>
            <div class="container py-5">
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <a class="btn btn-outline-secondary text-white" href="?week_offset=<?= $weekOffset - 1 ?>">&laquo;
                        Minulý týden</a>
                    <h3 class="m-0">Týden <?= $monday->format('j.n.Y') ?> – <?= $sunday->format('j.n.Y') ?></h2>
                        <div class="d-flex gap-2">
                            ...
                            <a class="btn btn-outline-secondary text-white"
                                href="?week_offset=<?= $weekOffset + 1 ?>">Příští týden &raquo;</a>
                        </div>
                </div>
                    ...
                    <?php foreach ([1, 2, 3, 4, 5] as $d): ?>
                        <div class="day-col">
                            ...
                            <?php foreach ($byDay[$d] as $blk): ?>
                                ...
                                ?>
                                <?php if (!empty($blk['term_id'])): ?>
                                    <a href="term_detail.php?id=<?= (int) $blk['term_id'] ?>">
                                    <?php else: ?>
                                        <a href="course_detail.php?id=<?= (int) $blk['course_id'] ?>">
                                        <?php endif; ?>

                                        <div class="event-block <?= htmlspecialchars($blk['term_css']) ?>"
                                            style="<?= $inlineStyle ?>">
                                            <div class="code"><?= htmlspecialchars($blk['course_code']) ?></div>
                                            <div class="name text-truncate"><?= htmlspecialchars($blk['course_name']) ?></div>
                                            <div class="meta">
                                                <?= $blk['start']->format('H:i') ?>–<?= $blk['end']->format('H:i') ?>
                                                · <?= htmlspecialchars($blk['type']) ?>
                                            </div>
                                        </div>
                                    </a>
                ...
                <?php if (empty($items)): ?>
                    <div class="alert alert-info mt-3"
                        style="background-color: var(--color-secondary); color: white; border: none;">Pro tento týden zatím
                        nemáš v rozvrhu nic.</div>
                <?php endif; ?>
            </div>
        </main>
    </div>
